...
module.exports = class guiService {
  ...
  static loadPermisos(context) {
    axios
      .patch(context.serviceUrl + '/api/permisos/' + permiso.id, permiso, context.config)
      .then(response => {
        spinner.succeed(chalk.green.bold('updated - ') + chalk.green(response.data.id));
      })
      .catch(error => {
        axios
          .post(context.serviceUrl + '/api/permisos', permiso, context.config)
          .then(response => {
            spinner.succeed(chalk.green.bold('created - ') + chalk.green(response.data.id));
          })
          .catch(errorCreated => {
            spinner.fail(chalk.green.bold('ups - ' + permiso.id + ': ') + chalk.green(errorCreated));
          });
      });
  }
};
